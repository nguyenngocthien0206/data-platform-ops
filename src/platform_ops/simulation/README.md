...
`platform-ops seed` writes nine tables into the `raw` schema of the DuckDB warehouse: customers, products, orders, order items, payments, web sessions, marketing campaigns, marketing spend and support tickets. Every value comes from a hash of the seed, table, row id and column, generated with SQL inside DuckDB and inserted in primary-key order. That combination is what makes two runs byte-identical, including the compressed storage layout the cost module will price. The reasoning is in [ADR 0004](../../../docs/adr/0004-deterministic-data-generation.md).
